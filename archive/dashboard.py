...
CONN = (
    ...
    "Trusted_Connection=yes;Connection Timeout=4;"
)
...
def q(sql: str) -> pd.DataFrame:
    try:
        with pyodbc.connect(CONN, timeout=4) as c:
...
def page_alerts(auth):
    ...
    total   = len(df)
    today_s_val = today_s()
    today_df = df[df["alerted_at"].astype(str).str[:10] == today_s_val] if total else pd.DataFrame()
    today_n  = len(today_df)
    avg_ch   = round(df["change_pct"].mean(), 1) if total and "change_pct" in df.columns else 0

    # Performance stats (alerts with outcome filled)
    ...
    if total:
        ...
        if not by_sess.empty:
            fig_s.add_trace(go.Bar(
                x=by_sess["session"], y=by_sess["n"], marker_color=YELLOW,
                text=by_sess["n"], textposition="outside", textfont=dict(color=TEXT),
            ))
        if not top_syms.empty:
            fig_t.add_trace(go.Bar(
                y=top_syms["symbol"], x=top_syms["n"], orientation="h",
                marker_color=CYAN,
    ...
    if not today_df.empty and "outcome" in today_df.columns:
        ...
        today_pend = today_df[today_df["outcome"].isna()]
        if not today_perf.empty or not today_pend.empty:
            rows = []
            for _, r in today_df.iterrows():
                ...
                rows.append(html.Tr([
                    ...
                    html.Td(f"${float(r['close_price']):.2f}" if r.get("close_price") is not None and str(r.get("close_price")) not in ("nan","None") else "—",
                             style={"padding": "8px 12px"}),
...
def page_trades(auth):
    ...
    # Build dropdown options from dates that actually have trades
    if is_admin:
        dates_df = q("SELECT DISTINCT CAST(trade_date AS DATE) AS d FROM trades ORDER BY d DESC")
    else:
        dates_df = q(f"SELECT DISTINCT CAST(trade_date AS DATE) AS d FROM trades WHERE chat_id = {uid} ORDER BY d DESC")
    ...
    filter_bar = html.Div(style={
        ...
        dcc.Dropdown(
            id="trades-filter-day",
            options=day_opts,
            value=day_opts[0]["value"],
            clearable=False,
            style=_DD_HIDE,
        ),
        dcc.Dropdown(
            id="trades-filter-month",
            options=month_opts,
            value=month_opts[0]["value"],
            clearable=False,
            style=_DD_HIDE,
        ),
    ])
...
def page_scan(auth):
    ...
    today_n = (len(df[df["scanned_at"].astype(str).str[:10] == today_s()])
               if total else 0)
        ...
        html.Div(id="login-section", style=_SHOW_LOGIN, children=[
            html.Div(style={
                ...
                dcc.Input(id="login-user", type="text",
                          ...
                          style=_INPUT),
                # PIN
                html.Label("PIN", style={
                    "color": MUTED, "fontSize": "10px", "fontWeight": "700",
                    "letterSpacing": "1px", "textTransform": "uppercase",
                    "display": "block", "marginBottom": "6px",
                }),
                dcc.Input(id="login-pin", type="password",
                ...
                # Hint
                html.P(
                    "Default PIN: 1234  ·  Change with /setpin in Telegram",
...
def update_trades_content(filter_type, sel_day, sel_month, auth):
    ...
    if filter_type == "day" and sel_day:
        period_where = f"CAST(t.trade_date AS DATE) = '{sel_day}'"
    elif filter_type == "month" and sel_month:
        y, m = sel_month.split("-")
        period_where = f"YEAR(t.trade_date) = {y} AND MONTH(t.trade_date) = {int(m)}"
...
if __name__ == "__main__":
    print("StockBot Dashboard  ->  http://localhost:8050")
